...
export function AppSidebar() {
  ...
  return (
    <Sidebar className="border-r border-[#C9A962]/20 bg-[#0B3D2C]">
      <SidebarResizeHandle />
      <div className="pt-4 flex flex-col h-full">
        ...
        <div className="flex-none px-3 mb-3">
          <Link
            href={"/"}
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-[#C9A962]/20 hover:border-[#C9A962]/40 hover:bg-[#C9A962]/10 transition-all duration-200 group"
          >
            <div className="relative">
              <div className="absolute -inset-1 bg-[#C9A962]/20 rounded-lg blur-sm" />
        ...
        <div className="flex-1 overflow-y-auto px-2">
            ...
            <SidebarMenu className="gap-1">
              {coreSections.map((section) => (
                <div key={section.title}>
                  {/* Section header */}
                  <div className="flex items-center gap-2 px-3 py-2 text-xs font-semibold text-[#C9A962]">
                    <section.icon className="w-4 h-4" />
                    <span className="uppercase tracking-wider font-[family-name:var(--font-body)]">{section.title}</span>
                  </div>
                  {/* Items */}
                  <SidebarMenuSub className="border-l-2 border-[#C9A962]/20 ml-2 mb-2">
                    {section.items.map((item) => (
                      <SidebarMenuSubItem key={item.title}>
                         <SidebarMenuButton
                          asChild
                          isActive={isActive(item.url)}
                          className="px-3 py-2 ml-1 rounded-lg text-sm transition-all duration-150 data-[active=true]:bg-[#C9A962]/20 data-[active=true]:text-[#C9A962] data-[active=true]:font-medium hover:bg-[#C9A962]/10 text-sidebar-foreground hover:text-sidebar-foreground"
                        >
                            <Link href={item.url} className="flex items-center gap-3">
                            <div className="relative">
                              <item.icon className="w-4 h-4 text-sidebar-foreground/60" />
                              {item.title === "FIRE Metrics" && fiPercent !== null && fiPercent >= 1 && (
                                <span className="absolute -top-1.5 -right-3 min-w-[18px] h-4 px-1 rounded-full bg-[#C9A962] text-[#0B3D2C] text-[9px] font-bold flex items-center justify-center leading-none">
                                  {fiPercent.toFixed(0)}%
                                </span>
                              )}
                            </div>
          ...
          <div className="flex items-center gap-2 px-3 py-2 mt-1">
            <Separator className="flex-1 bg-[#C9A962]/20" />
            <span className="text-[10px] uppercase tracking-widest text-[#C9A962]/60 font-semibold font-[family-name:var(--font-body)]">
              Extras
            </span>
            <Separator className="flex-1 bg-[#C9A962]/20" />
          </div>
                  ...
                  >
                    <SidebarMenuItem>
                      <CollapsibleTrigger asChild>
                        <SidebarMenuButton className="flex items-center gap-2 px-3 py-2 text-xs font-semibold text-sidebar-foreground/60 hover:text-sidebar-foreground hover:bg-[#C9A962]/10 rounded-lg transition-all duration-150">
                          <section.icon className="w-4 h-4" />
                      ...
                      <CollapsibleContent className="pt-1">
                         <SidebarMenuSub className="border-l-2 border-[#C9A962]/20 ml-2">
                          {section.items.map((item) => (
                            <SidebarMenuSubItem key={item.title}>
                               <SidebarMenuButton
                                asChild
...
<button
                ...
                <SidebarMenuButton
                  ...
                  className="px-3 py-2 rounded-lg text-sm transition-all duration-150 data-[active=true]:bg-[#C9A962]/20 data-[active=true]:text-[#C9A962] data-[active=true]:font-medium hover:bg-[#C9A962]/10 text-sidebar-foreground hover:text-sidebar-foreground"
                >
                ...
                <button
                  onClick={toggleTheme}
                  className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-sidebar-foreground/60 hover:text-[#C9A962] hover:bg-[#C9A962]/10 transition-all duration-150"
                >
